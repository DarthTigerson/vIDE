...
// in place), swaps the left panel to the file tree and asks it to reveal the
// file. Reads first, so an unreadable file leaves the panel and tabs untouched.
export async function openFileInTree(path: string, paneId: string): Promise<void> {
  ...
  useLeftPanelStore.getState().setPanel('files')
