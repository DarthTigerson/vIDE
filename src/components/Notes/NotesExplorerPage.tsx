...
export function NotesExplorerPage({ projectId }: { projectId: string }) {
  const project = useNotesStore((s) => s.projects.find((p) => p.id === projectId))
  ...
  return (
    <div className="h-full flex flex-col bg-panel overflow-hidden" onContextMenu={(e) => openContextMenu(e, null)}>
      <div className="h-11 px-4 border-b border-border shrink-0 flex items-center">
        <h1 className="text-sm font-semibold text-fg">{project.name}</h1>
      </div>

      <div className="flex-1 overflow-y-auto py-1">
        <NotesTree
          nodes={childrenByDir[rootPath] ?? []}
